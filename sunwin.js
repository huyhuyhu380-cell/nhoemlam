...
// ============================================================
// DATASET ANALYZER (self-learning)
// ============================================================
    ...
    trainOnHistory(historyRecords, epochs = 2) {
        ...
        for (let winLen = 5; winLen <= 15; winLen++) {
            for (let i = 0; i <= tx.length - winLen - 1; i++) {
                ...
                samples.push({ pattern: patternStr, next: nextResult === 'T' ? 't' : 'x' });
            }
        }
        if (samples.length === 0) {
...
class AdvancedDeepLearningAI {
    ...
    addResult(record) {
        // Chống trùng phiên
        if (this.history.some(h => h.session === Number(record.session))) return null;
...
async function pollHistory() {
    ...
    if (!rikCurrentSession || latestSession > rikCurrentSession) {
        ...
        if (isFirstLoad) {
            ...
            const existingSessions = new Set(ai.history.map(h => h.session));
            const newEntries = newHistory.filter(h => !existingSessions.has(h.session));
            newEntries.forEach(entry => ai.addResult(entry));
        }
    ...
    ws.on('message', (message) => {
        try {
            ...
            if (cmd === 1003 && gBB) {
                ...
                const record = {
                    session: Number(currentSessionId) || Date.now(),
                    dice: [d1, d2, d3],
                    total: total,
                    result: result
                };
                ...
                console.log('\n' + '🟦'.repeat(20));
                console.log(`🎲 Phiên ${currentSessionId} | KQ: ${result} (${total})`);
                console.log(`🤖 Dự đoán phiên tiếp: **${prediction.prediction.toUpperCase()}** (${(prediction.confidence*100).toFixed(1)}%)`);
                console.log(`📊 ${prediction.algorithms}/${ALGORITHMS.length} thuật toán tham gia`);
...
app.get('/api/sunwin/tx', (req, res) => {
    res.setHeader('Cache-Control', 'no-cache, no-store, must-revalidate');
    res.json(buildPredictionPayload());
});

app.get('/api/sunwin/tx/only', (req, res) => {
    const valid = rikResults.filter((r) => r.dice?.length === 3);
    const lastResult = valid.length ? valid[0] : null;
    const currentPrediction = ai.predict();
...
app.get('/api/sunwin/tx/history', (req, res) => {
    const valid = rikResults.filter((r) => r.dice?.length === 3);
...
app.get('/api/models', (req, res) => {
    res.json({
        ...
        weights: ai.algorithmWeights,
        pattern_database_size: Object.keys(PATTERN_DATABASE).length
    });
});
...
app.get('/', (req, res) => {
    res.json({
        ...
        version: "12.0 - WebSocket + API Hybrid",
...
const start = async () => {
    try {
        app.listen(PORT, '0.0.0.0', () => {
            console.log(`\n==============================================`);
            console.log(`🚀 @cskhgiabao Sunwin AI v12.0 (Hybrid)`);
            console.log(`==============================================`);
            ...
            console.log(`   API Source: kwinstore.com`);
            console.log(`   WebSocket: azhkthg1.net (realtime)`);
        ...
        console.log(`✅ Hệ thống chạy: API poll + WebSocket song song\n`);
    } catch (err) {
